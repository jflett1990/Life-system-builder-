...
Rules:
  ...
  CHAP_ACTION_MODE_WEAK      warning — insufficient action/section structures
...
class ActionModeCoverageRule(BaseRule):
    ...
    def check(self, stage_output: dict[str, Any], context: dict[str, Any]) -> list[Defect]:
        ...
        for chap in chapters:
            ...
            sections = chap.get("operational_sections") or []
            if len(mva) < 4 or len(sections) < 4:
                defects.append(self._defect(
                    stage=STAGE,
                    field_path=f"chapters[{num}]",
                    evidence=f"mva={len(mva)}, operational_sections={len(sections)}",
                    message=f"Chapter {num} under-serves scan/action mode; critical guidance may remain buried in prose.",
                    required_fix="Regenerate structure with stronger minimum_viable_actions and at least four operational_sections.",
                ))
        return defects
...
class DenseProseRule(BaseRule):
    ...
    def check(self, stage_output: dict[str, Any], context: dict[str, Any]) -> list[Defect]:
        ...
        for chap in chapters:
            num = chap.get("chapter_number", "?")
            narrative = chap.get("orientation_snapshot") or chap.get("narrative") or ""

            if not narrative.strip():
                defects.append(self._defect(
                    stage=STAGE,
                    field_path=f"chapters[{num}].orientation_snapshot",
                    evidence="missing orientation_snapshot",
                    message=f"Chapter {num} is missing orientation snapshot content.",
                    required_fix="Regenerate orientation snapshot with 2-3 concise paragraphs.",
                ))
