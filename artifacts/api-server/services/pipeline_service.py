...
class PipelineService:
    ...
    def _chapter_structure_defects(chapter: dict[str, Any]) -> list[str]:
        ...
        if len(chapter.get("minimum_viable_actions") or []) < 4:
            defects.append("minimum_viable_actions must contain at least 4 items")
        if len((chapter.get("orientation_snapshot") or "").split()) < 80:
            defects.append("orientation_snapshot must contain at least 80 words")
        if len(chapter.get("operational_sections") or []) < 4:
            defects.append("operational_sections must contain at least 4 sections")
        for idx, section in enumerate(chapter.get("operational_sections") or []):
            if not section.get("section_title"):
                defects.append(f"operational_sections[{idx}].section_title is required")
            if len(section.get("key_operational_considerations") or []) < 3:
                defects.append(f"operational_sections[{idx}] needs >=3 key_operational_considerations")
            if len(section.get("common_failure_modes") or []) < 3:
                defects.append(f"operational_sections[{idx}] needs >=3 common_failure_modes")
            if len(section.get("process_steps") or []) < 3:
                defects.append(f"operational_sections[{idx}] needs >=3 process_steps")
